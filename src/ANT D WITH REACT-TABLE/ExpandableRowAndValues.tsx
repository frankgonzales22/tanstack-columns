...
const ExpandableTable: React.FC = () => {
    ...
    // Dynamically get unique values from the first item in selectedColumnDrop
    const columnKey = selectedColumnDrop[0]; // Get the first item from selectedColumnDrop
    const yrSales = columnKey ? Array.from(new Set(data.map((item: DataItem) => item[columnKey]))) : [];

    // Create a mapping of selectedRowDrop[0] to selectedColumnDrop[0] to adecom and cvecom
    const aggregatedData = data.reduce((acc: any, item: any) => {
        ...
        if (!acc[rowKey][item[columnKey]]) {
            acc[rowKey][item[columnKey]] = {
                adecom: 0,
                cvecom: 0
            };
        }
        acc[rowKey][item[columnKey]].adecom += item.adecom;
        acc[rowKey][item[columnKey]].cvecom += item.cvecom;
        return acc;
