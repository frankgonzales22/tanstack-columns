...
import { MdOutlineBarChart, MdStackedBarChart } from "react-icons/md";
    ...
    return (
      <Box bgColor="white" opacity={0.96} shadow="xl" borderRadius={10}>
        <Text
          ...
          color="gray.600"
        >
          {`${label} ${payload[0].payload.column}`}
        </Text>
        <Box m={2}>
...
const ReusableStackedBarGraphForReportBuilder = ({
  data,
  headerTitle,
  columns,
  barColorArray,
  ...
  const customLegendArray2 = columns.map((a, index) => ({
    icon: <MdStackedBarChart size={"1.5rem"} />,
    key: index,
...
export default ReusableStackedBarGraphForReportBuilder;
